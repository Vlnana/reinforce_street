...
def train_rl_matcher_fast(data_dir, num_epochs=50, batch_size=4, save_dir='./checkpoints', reward_type='distance', num_workers=4, grad_accum_steps=4, use_amp=True, memory_monitor=True):
    # 数据预处理
    ...
    for epoch in range(num_epochs):
        ...
        for street_batch, satellite_batch in pbar:
            ...
            for i in range(min(len(street_images), len(satellite_labels))):
                ...
                # 每处理10个样本清理一次显存
                if i > 0 and i % 10 == 0 and device.type == 'cuda':
                    torch.cuda.empty_cache()
